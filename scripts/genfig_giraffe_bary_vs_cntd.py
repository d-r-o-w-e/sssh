...
def render_giraffe(V, F, outpath, numpy_scalar, si, cmapname, vminmax=None):
    ...
    meshpos = (0.71, 0.01, 0.13) # UI: click mesh > Transform > Location
    meshrot = (90, 0, 127) # UI: click mesh > Transform > Rotation
    meshscale = (0.45, 0.45, 0.45) # UI: click mesh > Transform > Scale
    ...
    mesh = vertexScalarToUV(mesh, numpy_scalar, vminmax) # using non-bt vertexscalartoUV
...
modelname = "giraffe"
modelfilename = "../models/giraffe.obj"
...
outpng_gt = "../results/pngs/giraffe_interp_gt.png"
outpng_ours_centroid = "../results/pngs/giraffe_centroid_ours.png"
outpng_ours_circum = "../results/pngs/giraffe_circum_ours.png"
outnpy = "../results/npys/giraffe_interp.npy"

f = heat_ridge_alpha(V, F, [0], 0.31)
...
if regen:
    ...
    # circumcenter
    st = time.time()
    recovered_fn_circum = hessian_l1_solve(hel, F, mode=hesstype, center="circumcenter", y=si, k=sv)
    et = time.time()
...
vmm = (f.min(), f.max())
render_giraffe(V, F, outpng_gt, f, si, cmaps[0], vminmax=vmm)
render_giraffe(V, F, outpng_ours_centroid, recovered_fn_centroid, si, cmaps[3], vminmax=vmm)
render_giraffe(V, F, outpng_ours_circum, recovered_fn_circum, si, cmaps[3], vminmax=vmm)