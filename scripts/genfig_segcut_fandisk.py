...
def render_seg(Vis, Fis, adjs, outpath):
    ...
    for i in range(len(Vis)):
        cidx = (3*i) % 10 # i + 1 to rotate the colors
        cidx_orig = (3*i) % 10
...
modelfilename = "../models/fandisk.obj"

# rotate
R1 = np.array([[1, 0, 0],
                [0, 0.7071068, 0.7071068],
                [0, -0.7071068, 0.7071068]])
R2 = np.array([[0.7071068, 0, 0.7071068],
                [0, 1, 0],
...
meanV = np.mean(V, axis=0, keepdims=True)

V = V - meanV

V = (R @ V.T).T

V, F = gp.subdivide(V, F, method='upsample')
...
eps2 = 0.01 # ending epsilon in A-T formula
